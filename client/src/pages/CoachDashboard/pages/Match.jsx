...
import { Search, User, Users, Shield, Send, Activity, Camera, X, ArrowLeft, Plus, Check, Upload, Loader2, Calendar, ChevronRight } from 'lucide-react';
...
const Match = () => {
    ...
    const handleOcrUpload = async (e) => {
        ...
        try {
            ...
            const { data } = await Tesseract.recognize(file, 'fra+eng', {
                logger: (m) => {
                    if (m.status === 'recognizing text') {
                        setOcrProgress(Math.round(5 + m.progress * 80));
            ...
            showNotification(err.response?.data?.message || "Tactical scan failed.", "error");
        } finally {
        ...
        <div className="overview-container dashboard-fashion-theme" style={{ padding: '0' }}>
                        ...
                        {matches.map((match, idx) => {
                            ...
                            return (
                                <div
                                    ...
                                    className={`intel-card ${isActive ? 'active-border-glow' : ''} ${isPast ? 'past-match-card' : ''} ${isSaved ? 'saved-match-card' : ''}`}
                                    style={{
                                        flex: '0 0 300px',
                                        background: isSaved ? 'rgba(219, 10, 64, 0.05)' : (isActive ? 'linear-gradient(135deg, rgba(219, 10, 64, 0.1) 0%, #111 100%)' : 'rgba(255,255,255,0.02)'),
                                        border: isSaved ? '1px solid #DB0A40' : (isActive ? '1px solid #DB0A40' : '1px solid rgba(255,255,255,0.05)'),
                                        padding: '1.5rem',
                                        cursor: 'pointer',
                                    ...
                                    onMouseLeave={(e) => {
                                        if (!isActive && !isSaved) {
                                            ...
                                            e.currentTarget.style.borderColor = isSaved ? '#DB0A40' : 'rgba(255,255,255,0.05)';
            ...
            <TacticalModal isOpen={showReportModal && selectedReportMatch} onClose={() => setShowReportModal(false)}>
                {selectedReportMatch && [
                    ...
                    <div key="sidebar" style={{ background: 'rgba(255,255,255,0.02)', padding: '3rem 2rem', borderRight: '1px solid rgba(255,255,255,0.05)', display: 'flex', flexDirection: 'column', gap: '2rem', height: '100%', overflowY: 'auto', minHeight: 0 }} className="full-custom-scroll">
                    ...
                    <div key="composition" style={{ padding: '3rem 2.5rem', display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', minHeight: 0 }}>
                        {
                            viewingImage ? (
